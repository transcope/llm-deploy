...
用法:
    # 下载校准数据集 (量化前预拉取)
    python scripts/hf_download.py --dataset neuralmagic/LLM_compression_calibration \\
        --save_dir /volume/hf_cache

    # 下载模型
    python scripts/hf_download.py --model Qwen/Qwen2.5-7B-Instruct \\
        --save_dir /volume/models

    # 用官方站点 (不走镜像)
    python scripts/hf_download.py --dataset <name> --save_dir <dir> --use_mirror False
